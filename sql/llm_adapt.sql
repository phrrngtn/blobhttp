...
-- prompt template via bt_template_render() (inja/Jinja2, from blobtemplates),
...
--
-- Requires: bhttp extension (this file), blobtemplates extension (bt_template_render).
--
...
CREATE OR REPLACE MACRO llm_adapt(adapter_name, params) AS TABLE (
    ...
    RENDERED AS (
        SELECT
            bt_template_render(prompt_template, params) AS prompt_text,
            output_schema,
            response_jmespath,
            max_tokens_str
        FROM ADAPTER_ROW
